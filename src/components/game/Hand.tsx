...
export function Hand({ cards, selectedCardId, disabled = false, onCardClick }: HandProps) {
  ...
  const overlapPercent = Math.min(60, 100 - (cards.length * 8));

  return (
    <div className="flex justify-center items-end">
      <div className="flex" style={{ marginLeft: `-${overlapPercent / 2}%` }}>
    ...
    <div className="flex justify-center items-end gap-2">
      {[...Array(cardCount)].map((_, index) => (
        <button
          ...
          className={cn(
            "w-24 h-36 rounded-lg shadow-md border-2 border-blue-800",
        ...
        >
          <div className="text-center text-blue-300">
            ...
            <div className="text-xs">Carta {index + 1}</div>
          </div>
        </button>
      ))}
    </div>
